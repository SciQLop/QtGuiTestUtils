...
\
template <class T>\
struct _has_##method<T, std::void_t<std::is_member_function_pointer<decltype(&T::method)>>>\
       : std::true_type{};\
\
template< class T>\
...
template <typename T>
void drag_widget(T* widget, int dx)
{
    auto cent = center(widget);
    QTest::mousePress(widget, Qt::LeftButton, Qt::NoModifier, cent, 1);
    mouseMove(widget, {cent.x() + dx, cent.y()}, Qt::LeftButton);
    QTest::mouseRelease(widget, Qt::LeftButton);
}

template<typename T>
bool prepare_gui_test(T* widget)
{
    widget->setGeometry(QRect(QPoint(QApplication::desktop()->geometry().center() - QPoint(250, 250)),
    QSize(500, 500)));
    widget->show();
